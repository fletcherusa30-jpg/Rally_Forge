...
```
  ...
  --wrap                        Embed everything in a big function, making the
                                â€œexportsâ€ and â€œglobalâ€ variables available. You
...
CoffeeScript â†’ compiled JS, UglifyJS can generate a map from CoffeeScript â†’
...
- `toplevel` â€” mangle names declared in the toplevel scope (disabled by
  default).
- `eval` â€” mangle names visible in scopes where `eval` or `with` are used
  (disabled by default).
When mangling is enabled but you want to prevent certain names from being
mangled, you can declare those names with `--reserved` (`-r`) â€” pass a
...
arguments) â€” in this case it will exclude names from all those files.
...
- `comparisons` -- apply certain optimizations to binary nodes, for example:
  `!(a <= b) â†’ a > b` (only when `unsafe_comps`), attempts to negate binary
  nodes, e.g. `a = !b && !c && !d && !e â†’ a=!(b||c||d||e)` etc.
...
- `new Array(1, 2, 3)` or `Array(1, 2, 3)` â†’ `[ 1, 2, 3 ]`
- `new Object()` â†’ `{}`
- `String(exp)` or `exp.toString()` â†’ `"" + exp`
- `new Object/RegExp/Function/Error/Array (...)` â†’ we discard the `new`
- `typeof foo == "undefined"` â†’ `foo === void 0`
- `void 0` â†’ `undefined` (if there is a variable named "undefined" in
...
```
Other options:
- `warnings` (default `false`) â€” pass `true` to display compressor warnings.
- `fromString` (default `false`) â€” if you pass `true` then you can pass
  JavaScript source code, rather than file names.
- `mangle` (default `true`) â€” pass `false` to skip mangling names, or pass
  an object to specify mangling options (see below).
- `mangleProperties` (default `false`) â€” pass an object to specify custom
  mangle property options.
- `output` (default `null`) â€” pass an object if you wish to specify
  additional [output options][codegen].  The defaults are optimized
  for best compression.
- `compress` (default `{}`) â€” pass `false` to skip compressing entirely.
  Pass an object to specify custom [compressor options][compressor].
- `parse` (default {}) â€” pass an object if you wish to specify some
...
##### mangleProperties options
 - `regex` â€” Pass a RegExp to only mangle certain names (maps to the `--mangle-regex` CLI arguments option)
 - `ignore_quoted` â€“ Only mangle unquoted property names (maps to the `--mangle-props 2` CLI arguments option)
 - `debug` â€“ Mangle names with the original name still present (maps to the `--mangle-props-debug` CLI arguments option). Defaults to `false`. Pass an empty string to enable, or a non-empty string to set the suffix.
We could add more options to `UglifyJS.minify` â€” if you need additional
...
- `strict` â€” disable automatic semicolon insertion and support for trailing
  comma in arrays and objects
- `bare_returns` â€” Allow return outside of functions. (maps to the
  `--bare-returns` CLI arguments option and available to `minify` `parse`
  other options object)
- `filename` â€” the name of the file where this code is coming from
- `toplevel` â€” a `toplevel` node (as returned by a previous invocation of
  `parse`)
...
â€œCompressor optionsâ€.  Defaults should lead to best compression in most
scripts.
...
most of them documented above in section â€œBeautifier optionsâ€.  The two
...
- `line` and `col`: the line and column where this comment appears in the
  original code
- `file` â€” the file name of the original file
- `nlb` â€” true if there was a newline before this comment in the original
