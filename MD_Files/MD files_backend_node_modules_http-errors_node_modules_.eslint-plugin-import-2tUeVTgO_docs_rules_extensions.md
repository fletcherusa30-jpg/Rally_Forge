...
```
 For example `{ "js": "always", "json": "never"Â }` would always enforce the use of the `.js` extension but never allow the use of the `.json` extension.
...
|      `action`    |    â˜‘ï¸     | `"enforce" \| "ignore"` | What action to take on imports whose specifiers match `pattern` |
### Exception
...
â”œâ”€â”€ foo
â”‚Â Â  â”œâ”€â”€ bar.js
â”‚Â Â  â”œâ”€â”€ bar.json
```
and this import statement:
```js
import bar from './foo/bar.json';
```
then the extension canâ€™t be omitted because it would then resolve to `./foo/bar.js`.
### Examples
