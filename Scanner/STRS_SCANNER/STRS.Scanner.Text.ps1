# ============================================================
# DEPRECATED — STRS.Scanner.Text.ps1
# ============================================================
# DEPRECATION NOTICE:
...
. "$PSScriptRoot\engine\STRS.Utils.ps1"
. "$PSScriptRoot\engine\STRS.Extractor.ps1"
. "$PSScriptRoot\engine\STRS.Analyzer.ps1"
. "$PSScriptRoot\engine\STRS.Output.ps1"
. "$PSScriptRoot\engine\STRS.NLP.ps1"

$raw = Get-Content -Path $TextPath -Raw
$normalized = $raw -replace '\s{3,}', ' ' `
               -replace ' +', ' ' `
               -replace '\r', '' `
               -replace '\n{2,}', "`n"

$extracted = Invoke-STRSExtraction -Text $normalized
$analysis  = Invoke-STRSAnalysis -Data $extracted
$nlp       = Invoke-STRSNLP -Text $normalized -Extracted $extracted

Invoke-STRSOutput -Extracted $extracted -Analysis $analysis -NLP $nlp -OutFile $OutputJson
