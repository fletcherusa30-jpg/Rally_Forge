...
function Invoke-STRSAnalysis {
    ...
    $events = @($Data.Events)
    $dates = @($Data.Dates)
    $pages = @($Data.PageNumbers)

    # COPILOT CHANGE: Enhanced diagnosis detection
    if ($diagnoses.Count -gt 0) {
        $flags += "Medical diagnoses present: $($diagnoses.Count) diagnosis line(s) identified"
        
        # Check for chronic conditions
        $chronicCount = ($diagnoses | Where-Object { ([string]$_.text) -match 'chronic|ongoing|persistent' }).Count
        if ($chronicCount -gt 0) {
            $flags += "Chronic condition pattern: $chronicCount chronic diagnoses"
        }
    }

    if ($symptoms.Count -gt 0) {
        $flags += "$($symptoms.Count) symptom line(s) documented"
    }

    if ($events.Count -gt 0) {
        $flags += "$($events.Count) event/injury line(s) documented"
    }

    if ($dates.Count -gt 0) {
        $flags += "$($dates.Count) date reference(s) extracted"
    }

    if ($pages.Count -gt 0) {
        $flags += "$($pages.Count) page marker(s) extracted"
    }

    # COPILOT CHANGE: Service connection opportunity detection
    if ($diagnoses.Count -gt 0 -and $events.Count -gt 0) {
        $issues += @{
            Type = "Service_Connection_Opportunity"
            FindingCount = $diagnoses.Count
            Summary = "Diagnosis and in-service event patterns appear in the same STR corpus"
        }
    }

    # COPILOT CHANGE: Continuity/chronicity detection
    $chronicity = ($diagnoses | Where-Object { ([string]$_.text) -match 'chronic|ongoing|persistent' }).Count
    if ($chronicity -gt 0) {
        $issues += @{
            Type = "Chronicity_Detected"
            FindingCount = $chronicity
            Summary = "$chronicity condition(s) showing chronic pattern across multiple visits"
        }
    }
    ...
    return @{
        ...
        PageReferenceCount = $pages.Count
        EncounterCount   = 0
        MedicationCount  = 0
        ImagingCount     = 0
        LabCount         = 0
        Summary          = "STRS review completed with $($flags.Count) significant findings and $($issues.Count) issues identified."
    }
}
